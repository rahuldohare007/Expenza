...
export default function ExpensesScreen() {
  ...
  const fetchBudget = async () => {
    try {
      ...
      setBudget(response.data);
      updateBudgetItem(response.data); // Notify parent component about budget update
    } catch (error) {
      console.error("Error fetching budget:", error);
    }
  };
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const updatedBudget = { ...budget, totalSpend, totalItem };
      setBudget(updatedBudget);
      updateBudgetItem(updatedBudget); // Notify parent component about budget update
    } catch (error) {
      console.error("Error fetching expenses:", error);
    }
  };
  ...
  const handleDeleteExpense = (deletedExpense) => {
    const newExpenses = expenses.filter(expense => expense._id !== deletedExpense._id);
    setExpenses(newExpenses);
    const newTotalSpend = newExpenses.reduce(
      (sum, expense) => sum + expense.ExpenseAmount,
      0
  ...
  return (
    <div className="p-10">
      ...
      <div className="mt-4">
        <h3 className="font-bold text-lg">Latest Expenses</h3>
        <div>
          {expenses.length > 0 ? (
            <ExpenseListTable expenses={expenses} onExpenseDeleted={handleDeleteExpense}/>
          ) : (
            <p className="text-red-500">No expenses found for this budget.</p>
          )}
        </div>
